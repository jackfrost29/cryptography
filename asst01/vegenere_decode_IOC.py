...
freq_count = {}
for i in range(26):
    freq_count[chr(i+ord('a'))] = 0.

for gues_len in range(2, cypher_len):
    count = 0
    j = 0
    while (j < cypher_len):
        freq_count[cypher[j]] += 1.
        j += gues_len
        count += 1
    nm = 0.
    for key in freq_count:
        if freq_count[key] != 0:
            freq_count[key] *= (freq_count[key]-1)

        nm += freq_count[key]
        freq_count[key] = 0.
    IOC[gues_len] = abs(nm / (count * (count-1)) - IC_english)
